...
const Schedules = () => {
  ...
  useEffect(() => {
    ...
    if (destinationAirport) {
      filtered = filtered.filter(
        schedule => schedule.destinationAirportResponse?.id === destinationAirport.id
      );
    }
    ...
    if (flightNumber.trim() !== "") {
      ...
      filtered = filtered.filter(
        schedule =>
          schedule.flightResponse?.flightNumber.toString().toLowerCase().includes(searchTerm) ||
  ...
  useEffect(() => {
    if (airports.length > 0) {
      ...
      if (initialSourceId) {
        const source = airports.find(airport => airport.id === parseInt(initialSourceId));
        if (source) setSourceAirport(source);
      }

      // Set destination airport if provided in URL
      if (initialDestinationId) {
        const destination = airports.find(airport => airport.id === parseInt(initialDestinationId));
      ...
      <div className="mb-8">
        <div className="w-full bg-white rounded-lg shadow mb-4">
          ...
          >
            ...
            <svg
              className={`w-6 h-6 transition-transform ${isFiltersOpen ? 'transform rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <div className="p-4 border-t">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-2 flex flex-col justify-center items-center">
                  <label className="block text-sm font-semibold text-gray-700 w-full text-left">Select Date Range</label>
                  <DatePicker setDate={handleDateSelect} setDay={setDay} />
                </div>

                {/* Source Airport */}
                <div className="space-y-2 flex flex-col justify-center items-center">
                  <label className="block text-sm font-semibold text-gray-700 w-full text-left">From</label>
                  <CustomAutocomplete
                    value={sourceAirport}
                ...
                <div className="space-y-2 flex flex-col justify-center items-center w-full">
                  <label className="block text-sm font-semibold text-gray-700 w-full text-left">To</label>
                  <CustomAutocomplete
          ...
          {filteredSchedules.length > 0 ? (
            <div className="md:w-[80%] w-full mx-auto my-2">
              {filteredSchedules.map((schedule, index) => (
                <Card key={index} className="overflow-hidden my-4 hover:shadow-lg transition-shadow">
                  <CardContent className="p-4">
                    <div className="flex flex-col">
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <span className="font-bold text-lg">{format(new Date(schedule.dateTime), "dd MMMM, yyyy")}</span>
                          <span className="ml-2 text-brandColor font-semibold">{format(new Date(schedule.dateTime), "h:mm a")}</span>
                        </div>
                        <div className="text-right">
                          <p className="text-sm text-gray-500">Flight</p>
                          <p className="font-semibold">{schedule.flightResponse?.company} #{schedule.flightResponse?.flightNumber}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-4">
                        <div className="p-3 bg-gray-50 rounded-lg">
                          ...
                          <p className="font-semibold">{schedule.sourceAirportResponse?.city}</p>
                          <p className="text-sm text-gray-600 truncate" title={schedule.sourceAirportResponse?.name}>
                            {schedule.sourceAirportResponse?.name}
                          </p>
                        </div>
                        <div className="p-3 bg-gray-50 rounded-lg">
                          <p className="text-sm text-gray-500">To</p>
                          <p className="font-semibold">{schedule.destinationAirportResponse?.city}</p>
                          <p className="text-sm text-gray-600 truncate" title={schedule.destinationAirportResponse?.name}>
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex space-x-4">
                          <div>
                            ...
                            <p className="font-semibold text-green-600">{schedule.flightResponse ?
                              (schedule.flightResponse.totalSeats - schedule.flightResponse.bookedSeats) : "N/A"}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Booked</p>
                            <p className="font-semibold text-red-600">{schedule.flightResponse?.bookedSeats || "N/A"}</p>
                          </div>
                        </div>

                        {schedule.flightResponse &&
                          (schedule.flightResponse.totalSeats - schedule.flightResponse.bookedSeats) > 0 ? (
                          <a
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-500 mt-2">Try selecting a different date or changing your search criteria.</p>
            </div>
          )}
        </div>
      )}
